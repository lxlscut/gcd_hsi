...
def get_HSI_patches(x, gt, ksize, stride=(1, 1), padding='reflect', is_index=False, is_labeled=True):
    ...
    print('x_patches shape: %s, labels: %s' % (x_patches.shape, np.unique(y_patches_nonzero)))
...
class Load_my_Dataset():
    ...
    def __init__(self, image_path, label_path, patch_size, band_number, device):
        ...
        x_train, y_patches, lo_index = get_HSI_patches(x=X, gt=Y, ksize=(patch_size, patch_size), is_labeled=True)
        ...
        self.y, self.train, self.index = np.array(y_patches), np.array(x_train), np.array(lo_index)

    def __len__(self):
        return self.train.shape[0]

    def __getitem__(self, idx):
        return self.train[idx], self.y[idx],
